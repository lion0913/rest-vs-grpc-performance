...
usage() {
    ...
    echo "  --basic-multiple    기본 테스트 10회 반복 (30초 간격)"
    echo "  --latency-multiple  네트워크 지연 테스트 10회 반복 (30초 간격)"
    ...
    echo "  $0 --basic"
    echo "  $0 --latency"
    echo "  $0 --basic-multiple"
    echo "  $0 --latency-multiple"
    echo "  $0 --stop"
}
...
# 기본 테스트 10회 반복
run_basic_test_multiple() {
    echo "Starting basic test (no latency) - 10 runs with 30s interval..."
    echo ""
    ...
    # 테스트 실행
    echo "Running performance test (10 runs)..."
    echo "This will take approximately 8-10 minutes..."
    echo ""

    curl -X POST "http://localhost:8080/api/test/compare-multiple?totalCount=400000&batchSize=1000&runs=10&intervalSeconds=30&withLatency=false"
    ...
    echo ""
    echo "Check results:"
    echo "  cat docs/response-basic-multiple.md"
    echo ""
    echo "View logs:"
    echo "  docker-compose logs service-a"
    echo "  docker-compose logs service-b"
...
# 네트워크 지연 테스트 10회 반복
run_latency_test_multiple() {
    echo "Starting latency test (10ms ± 5ms) - 10 runs with 30s interval..."
    echo ""
    ...
    # 테스트 실행
    echo "Running performance test with network latency (10 runs)..."
    echo "This will take approximately 10-12 minutes..."
    echo ""

    curl -X POST "http://localhost:8080/api/test/compare-multiple?totalCount=400000&batchSize=1000&runs=10&intervalSeconds=30&withLatency=true"
    ...
    echo ""
    echo "Check results:"
    echo "  cat docs/response-latency-multiple.md"
    echo ""
    echo "Compare with basic test results to see the impact of network latency"
    echo ""
    echo "View logs:"
